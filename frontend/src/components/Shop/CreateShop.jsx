  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setForm({ name: '', email: '', password: '', address: '', phoneNumber: 0, zipCode: 0 });
        setAvatar(null);
      })
      .catch((err) => {
      ...
      <div className='w-lg m-3 sm:m-0 sm:w-lg md:w-md lg:w-md bg-white rounded-md shadow-lg shadow-gray-300'>
        <form onSubmit={handleSubmit} className='flex justify-center flex-col gap-2 p-4 sm:p-6'>
          <div className='flex items-center'>
            ...
            <h1 className='font-semibold text-2xl'>Ecommerce Platform</h1>
          </div>
          <h2 className='font-medium text-gray-400'>Enter credentials To create Account</h2>
          <div className='flex flex-col gap-2'>
            <label htmlFor="name" className='font-medium text-gray-500'>Shop Name</label>
            <input type="text"
              name='name'
              id='name'
              required
          ...
          <div className='flex flex-col gap-2'>
            <label htmlFor="phoneNumber" className='font-medium text-gray-500'>Phone Number</label>
            <input type="Number"
              name='phoneNumber'
              id='phoneNumber'
              required
          ...
          <div className='flex flex-col gap-2'>
            <label htmlFor="phoneNumber" className='font-medium text-gray-500'>Zip Code</label>
            <input type="Number"
              name='zipCode'
              id='zipCode'
              required
          ...
          <div className='flex gap-2 text-sm'>
            ...
            <Link to="/login-shop" className='font-semibold text-blue-700'>Login</Link>
          </div>
        </form>
      </div>
    </div>
